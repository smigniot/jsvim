...
# endif
    ...
    //JSVIM while (1) { xvi_loop(); }
    EM_ASM({vim_eventLoop();});
}
...
tty_startv()
{
    ...
    EM_ASM({console.log("DBG_before VSset_colour");});
    VSset_colour(&tcap_scr, VSCcolour);
    EM_ASM({console.log("DBG_after VSset_colour");});
    optimise = FALSE;
    termmode = m_VI;
}
...
delete_a_line()
{
    EM_ASM({console.log("DBG: delete_a_line")});
    xyupdate();
    if (DL != NULL)
...
erase_display()
{
    EM_ASM({console.log("DBG: erase_display")});
    /*
...
int	row, col;
{
    //EM_ASM_({console.log("DBG: tty_goto row,col",$0,$1)},row,col);
    virt_row = row;
    virt_col = col;
}
