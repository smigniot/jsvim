...
sys_startv()
{
    EM_ASM({console.log("DBG_at_sys_startv");});
    if (curmode == m_VI)
	return;
#ifdef	TERMIO
    /*
    EM_ASM({console.log("DBG_TERMIO defined");});
    EM_ASM_({console.log("DBG_raw_state c_i", $0);}, raw_state.c_iflag);
    EM_ASM_({console.log("DBG_raw_state c_o", $0);}, raw_state.c_oflag);
    EM_ASM_({console.log("DBG_raw_state c_c", $0);}, raw_state.c_cflag);
    EM_ASM_({console.log("DBG_raw_state c_l", $0);}, raw_state.c_lflag);
    for(int i=0; i<NCCS; i++) {
        EM_ASM_({console.log("DBG_raw_state c_cc[?]=?", $0, $1);}, i, raw_state.c_cc[i]);
    }
    */
    //JSVIM here the state is assigned.
    // w_setstate(&raw_state);
    //EM_ASM({console.log("No need to assign struct termios, skipping 'w_setstate(&raw_state);' check all are zeros except c_cc[6] = 1");});
#else
    ...
    EM_ASM({console.log("DBG_before_tty_startv");});
    tty_startv();
    EM_ASM({console.log("DBG_after_tty_startv");});
