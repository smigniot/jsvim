    ...
    char	*str;
{
	...
	return NULL;
    //JSVIM if ((retp->rn_ptr = regcomp(str)) == NULL) {
    if ((retp->rn_ptr = vim_regcomp(str)) == NULL) {
	free ((char *) retp);
	return NULL;
    }
    retp->rn_count = 1;
...
bool_t		matchtype;
{
	...
	break;
    default:
	vim_regerror("Invalid command character");
	return;
    }
    ...
    ndone = 0;
    while (lp != up) {
	if (matchtype == vim_regexec(prog, lp->l_text, TRUE)) {
	    Line	*thisline;
...
grep_line()
{
	...
	if (greptype == vim_regexec(prog, curline->l_text, TRUE)) {
...
unsigned long	lnum;
{
	...
	vim_regerror("NULL parameter to regsubst");
	return;
    }

    if (UCHARAT(prog->program) != MAGIC) {
	vim_regerror("Damaged regexp fed to regsubst");
	return;
    }
	...
	if (no < 0) {
		...
		if (*cp == '\0') {
		    vim_regerror("Damaged match string");
		    return;
		} else {
...
char	*command;
{
	...
	vim_regerror("Usage: :s/search pattern/replacement/");
	free(copy);
	return(0);
    }
    sub = cp;
...
char	*flags;
{
	...
	if (vim_regexec(prog, lp->l_text, TRUE)) {
	    char	*p, *matchp;
	    ...
	    } while (do_all && vim_regexec(prog, matchp, FALSE));
